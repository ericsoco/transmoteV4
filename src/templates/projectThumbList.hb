			...
			<div class="project-thumb{{#if featured}} featured{{/if}}" data-project="{{id}}">
				{{#ifVideo media.[0]}}
					{{#if media.[0].poster}}
						...
						<div class="project-img" title={{name}} style="background-image: url({{concatPath "projects" id media.[0].poster}});"></div>
					{{else}}
				...
				{{else}}
					...
					<div class="project-img" title={{name}} style="background-image: url({{concatPath "projects" id media.[0].path}});"></div>
				{{/ifVideo}}
